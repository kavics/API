...
namespace Tests
{
    ...
    public class GatheringTests
    {
        ...
        public void Api_Types_Public()
        {
            ...
            const string expected = "IPub, PubAC, PubC, PubEnum, PubNCofPub, PubSeC, PubStC, PubStruct";
        ...
        public void Api_Types_All()
        {
            ...
            const string expected = "IInt, IntAC, IntC, IntEnum, IntNCofInt, IntNCofPub, IntSeC, IntStC, IntStruct, IPub, " +
        ...
        public void Api_Members_Public()
        {
            ...
            const string expectedFields = "_protectedField, _protectedInternalField, _publicField";
            var actualFields = string.Join(", ", types[0].Fields.Select(f => f.Name).OrderBy(s => s));
            Assert.AreEqual(expectedFields, actualFields);

            const string expectedProperties = "P1, P2, P4, P5, P6, P7";
            ...
            const string expectedMethods = "M1, M2, M4, MA0, MA1, MA3, MS1, MS2, MS4, MV1, MV2, MV4, ToString";
        ...
        public void Api_Members_All()
        {
            ...
            const string expectedFields = "_internalField, _privateField, _protectedField, _protectedInternalField, _publicField";
            var actualFields = string.Join(", ", types[0].Fields.Select(f => f.Name).OrderBy(s => s));
            Assert.AreEqual(expectedFields, actualFields);

            const string expectedProperties = "P0, P1, P2, P3, P4, P5, P6, P7";
            var actualProperties = string.Join(", ", types[0].Properties.Select(f => f.Name).OrderBy(s => s));
            Assert.AreEqual(expectedProperties, actualProperties);

            const string expectedMethods = "M0, M1, M2, M3, M4, MA0, MA1, MA2, MA3, MS0, MS1, MS2, MS3, MS4, MV1, MV2, MV3, MV4, ToString";
            var actualMethods = string.Join(", ", types[0].Methods.Select(f => f.Name).OrderBy(s => s));
            Assert.AreEqual(expectedMethods, actualMethods);
        }
    }
